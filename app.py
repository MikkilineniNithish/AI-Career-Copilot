...
if resume and jd:
    ...
    # -------------------------
    # ROLE DETECTION (JD PRIORITY FIX)
    # -------------------------
    detected_role = None

    # 1️⃣ Detect from JD (highest priority)
    if "cloud" in jd_text:
        detected_role = "cloud engineer"
    elif "web developer" in jd_text or "frontend" in jd_text or "backend" in jd_text:
        detected_role = "web developer"
    elif "data scientist" in jd_text or "data science" in jd_text:
        detected_role = "data scientist"
    elif "ai engineer" in jd_text or "machine learning" in jd_text:
        detected_role = "ai engineer"

    # 2️⃣ If not found in JD → detect from resume
    ...
    st.session_state.logs.append(
        f"{datetime.now().strftime('%d-%m %H:%M')} | ATS:{ats_score}% | Skill:{skill_score}% | Role:{detected_role}"
    )
